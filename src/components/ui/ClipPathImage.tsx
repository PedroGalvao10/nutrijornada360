...
export function ClipPathImage({
  ...
  // id único por instância evita colisão se houver vários recortes na página
  const clipId = React.useId().replace(/:/g, '') + '-' + clip;

  return (
    <figure className={className} style={{ clipPath: `url(#${clipId})` }}>
      <svg className="absolute w-0 h-0" aria-hidden="true">
        <defs>
          <clipPath id={clipId} clipPathUnits="objectBoundingBox">
            <path fillRule="evenodd" clipRule="evenodd" d={CLIP_PATHS[clip]} />
          </clipPath>
        </defs>
      </svg>
      <img
